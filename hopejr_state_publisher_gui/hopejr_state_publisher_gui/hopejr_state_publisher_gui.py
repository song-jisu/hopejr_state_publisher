...
from hopejr_state_publisher.hopejr_state_publisher import HopejrStatePublisher, HOPEJR_MOTORS

from hopejr_state_publisher_gui.flow_layout import FlowLayout
...
class HopejrStatePublisherGui(QMainWindow):
    sliderUpdateTrigger = Signal()
    initialize = Signal()

    def __init__(self, title, jsp):
        super(HopejrStatePublisherGui, self).__init__()
        ...
        # Tell self to draw sliders in case the HopejrStatePublisher already has a robot_description
    ...
    def initializeSliders(self):
        ...
        # If the state publisher has not yet populated free_joints, defer initialization.
        if not hasattr(self.jsp, 'free_joints') or not self.jsp.free_joints:
            print('initializeSliders: waiting for jsp.free_joints to be available')
            return

        # reset slider registry
        self.sliders = {}

        # Build sliders for all HOPEJR_MOTORS entries. Even if motor name is not
        # an URDF free joint, create a motor-only slider using JSON min/max/default.
        for motor_name, mcfg in HOPEJR_MOTORS.items():
            backing_joint = None
            if motor_name in self.jsp.free_joints:
                backing_joint = self.jsp.free_joints[motor_name]
            else:
                # try explicit mapping.joint in JSON
                mapped = None
                if isinstance(mcfg, dict):
                    mapping = mcfg.get('mapping', {})
                    mapped = mapping.get('joint') if isinstance(mapping, dict) else None
                if mapped and mapped in self.jsp.free_joints:
                    backing_joint = self.jsp.free_joints[mapped]

            if backing_joint is None:
                # create a motor-only virtual joint object so slider UI can display ranges
                # prefer JSON min/max/default if provided, else fall back to reasonable values
                minv = -3.14159
                maxv = 3.14159
                default = 0.0
                if isinstance(mcfg, dict):
                    try:
                        ...
                        if 'max' in mcfg:
                            maxv = float(mcfg['max'])
                        if 'default' in mcfg:
                            default = float(mcfg['default'])
                    except Exception:
                        pass
                backing_joint = {'min': minv, 'max': maxv, 'position': default}
                motor_only = True
            else:
                motor_only = False

            # skip degenerate/fixed joints
            if backing_joint.get('min') == backing_joint.get('max'):
                continue

            slider = Slider(motor_name)
            self.joint_map[motor_name] = {
                'display': slider.display,
                'slider': slider.slider,
                'joint': backing_joint,
                'motor_only': motor_only
            }
            self.sliders[slider] = motor_name
            self.scroll_layout.addWidget(slider)
            # Connect slider change; capture motor_name in lambda
            slider.slider.valueChanged.connect(lambda event, name=motor_name: self.onSliderValueChangedOne(name))
    ...
    def onSliderValueChangedOne(self, name):
        """
        Called when a single slider changes.
        If the slider is motor_only, write its value to jsp.motors so the publisher uses it.
        Otherwise update the backing URDF joint position as before.
        """
        if name not in self.joint_map:
            return
        joint_info = self.joint_map[name]
        slider_widget = joint_info['slider']
        raw = slider_widget.value()
        # assume sliderToValue exists and converts slider int -> physical position
        val = self.sliderToValue(raw, joint_info['joint'])

        if joint_info.get('motor_only', False):
            # write into jsp.motors so timer_callback will pick it up
            try:
                self.jsp.motors[name] = float(val)
            except Exception:
                self.jsp.motors[name] = val
            joint_info['display'].setText("%.3f" % float(val))
        else:
            joint = joint_info['joint']
            joint['position'] = float(val)
            joint_info['display'].setText("%.3f" % float(val))

        # notify source_update / trigger immediate publish if desired
        try:
            # If HopejrStatePublisher exposes a publish helper, call it; else rely on timer
            if hasattr(self.jsp, 'publish_joint_states'):
                self.jsp.publish_joint_states()
        except Exception:
            pass
...
def main():
    ...
    jsp_gui = HopejrStatePublisherGui('Hopejr State Publisher',
                                     HopejrStatePublisher(parsed_args.urdf_file))
